...
const questionSchema = z.object({
  ...
  options: z.array(z.object({
    option_text: z.string(), // Remove min validation here, handle in submit
...
export default function CreateQuestionPage() {
  ...
  const { register, control, handleSubmit, watch, setValue, getValues, formState: { errors } } = useForm<QuestionFormData>({
  ...
  const onSubmit = async (data: QuestionFormData, resetForm: boolean = false) => {
    ...
    try {
      // Filter out empty options and add order
      const filteredOptions = nonEmptyOptions.map((option, index) => ({
        ...option,
        order: index + 1
      }));
